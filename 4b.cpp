...
#include <gmpxx.h>
=======
>>>>>>> aff33015126d81eaadd254dba41ad07c7cb46b7c
#include <iostream>
#include <vector>
#include <random>
...
mpz_class pollard_rho(const mpz_class& n) {
    ...
    while (d == 1 && iterations < max_iterations) {
        ...
        if (d == n) {
            // Если не удалось найти нетривиальный множитель, пробуем с другими начальными значениями
            x = dis(gen);
            y = x;
            c = dis(gen);
            d = 1;
...
void worker_thread(const mpz_class& N, int trials_per_thread, int thread_id) {
    ...
    for (int i = 0; i < trials_per_thread; i++) {
        ...
        if ((i + 1) % (trials_per_thread / 10) == 0 || trials_per_thread < 10) {
            std::lock_guard<std::mutex> lock(cout_mutex);
            std::cout << "Thread " << thread_id << ": " << (i + 1) * 100 / trials_per_thread 
                      << "% complete" << std::endl;
        }
    }
...
int main() {
    // Константы из условия задачи
    int M = 1000;  // Количество испытаний
    mpz_class Nrand("1000000000000000000000000000000000000000000000000000");  // 10^50
    ...
    // Определение количества потоков
    unsigned int num_threads = std::thread::hardware_concurrency();
    if (num_threads == 0) num_threads = 4; // Если не удалось определить, используем 4 потока
    ...
    for (mpz_class N = N_start; N <= N_end; N += 10) {
        ...
        for (unsigned int i = 0; i < num_threads; i++) {
            int thread_trials = trials_per_thread + (i < (unsigned int)remaining_trials ? 1 : 0);
            threads.push_back(std::thread(worker_thread, N, thread_trials, i));
        }
        ...
        // Выводим результаты
        std::cout << "Results for N = " << N << ":" << std::endl;
        std::cout << "  Squarefree numbers found: " << squarefree_count << " out of " << M << std::endl;
        std::cout << "  Experimental probability: " << experimental_prob << std::endl;
        std::cout << "  Theoretical probability (6/pi^2): " << theoretical_prob << std::endl;
        std::cout << "  Theoretical with O(1/sqrt(N)) correction: " << theoretical_with_correction << std::endl;
        std::cout << "  Difference: " << std::abs(experimental_prob - theoretical_prob) << std::endl;
        std::cout << "  Cache hits: " << cache_hits << std::endl;
        std::cout << "  Current cache sizes:" << std::endl;
        std::cout << "    Squarefree cache: " << squarefree_cache.size() << " entries" << std::endl;
        std::cout << "    Prime cache: " << prime_cache.size() << " entries" << std::endl;
        std::cout << "    Factor cache: " << factor_cache.size() << " entries" << std::endl;
